  ...
  return {
    ...
    update(world: TowerWorld, dt: number): void {
      ...
      for (let i = 0; i < entities.length; i += 1) {
        ...
        let duration = Burn.duration[eid] ?? 0;
        let tickTimer = Burn.tickTimer[eid] ?? 0;
        const tickInterval = Burn.tickInterval[eid] ?? 1;
        const damagePerTick = Burn.damagePerTick[eid] ?? 0;
        ...
        Burn.duration[eid] = duration;
        Burn.tickTimer[eid] = tickTimer;
      }
    },
  };
}
