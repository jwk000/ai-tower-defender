...
玩家进入卡池界面后，看到本 Run 当前卡组内的所有卡牌实例列表。每张卡牌显示：
- 卡牌基础信息（名称 / 类型 / 能量消耗）
- 已点亮节点数 / 装备路径名称
- **「技能」按钮**（点击进入该卡实例的技能树面板）
...
│                                                                 │
│  ┌─ 底部状态栏 ─────────────────────────────────────────────────┐│
│  │  本 Run 已点亮：5 节点 / 累计消耗 SP：14 / 剩余 SP：23       ││
│  │  [关闭]                                                      ││
│  └─────────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────┘
```
...
**卡牌项说明**：
- `#1` = 同种类多张卡时的实例序号（如有两张箭塔，分别显示 `#1` / `#2`，各自技能树独立）
- `✓` = 已装备路径名称
- `✨` = 该卡已有节点点亮（快速识别已强化卡）
- `[技能]` 按钮 = 进入该卡实例的技能树面板

### 9.3 技能树面板（从卡池点击「技能」进入）
...
| 卡牌「技能 ✨」按钮 | 已有点亮节点时高亮（金色边框），无点亮时普通样式 |

### 9.5 交互流程

**卡池界面：**
1. 进入界面 → 展示本 Run 卡组内所有卡牌实例，按类别分组（塔 / 士兵 / 陷阱 / 法术 / 生产）
2. 同一种类多张卡 → 各自独立显示（如两张箭塔显示「箭塔 #1」「箭塔 #2」，技能树各自独立）
3. 点击某卡的「技能」按钮 → 进入该卡实例的技能树面板

**技能树面板：**
1. 显示该卡实例的完整技能树（多路径并排展示）
2. 点击某节点 → 底部展开节点详情（名称 / 效果 / SP 单价 / 前置条件）
3. 点击「点亮 · N SP」→ 扣 SP / 节点高亮（邻接节点如满足前置则变为可点亮状态）
4. 点击「切换装备到路径 X」→ 切换装备（0 SP）
5. 点击「← 返回卡池」→ 返回卡池界面

**退出：**
6. 卡池界面点击「关闭」→ 返回关后路线图屏幕
...
- [ ] 卡池界面独立入口（关后路线图按钮，非 3 选 1 路径之一）
- [ ] 卡池界面显示本 Run 已获得的卡牌实例列表，按类别分组
- [ ] 同种类多张卡分别独立显示（箭塔 #1 / 箭塔 #2），各自技能树互不影响
- [ ] 卡牌项显示已点亮节点数 / 装备路径名称 / 「技能」按钮
- [ ] 有点亮节点的卡牌「技能 ✨」按钮高亮（金色边框）
- [ ] 点击「技能」按钮进入该卡实例的技能树面板
- [ ] 技能树面板顶部「← 返回卡池」按钮
