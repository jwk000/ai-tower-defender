...
**一切皆卡牌**。玩家可部署的所有内容（塔/兵/陷阱/建筑/法术）都通过出卡触发。卡是"召唤令"，使用一次即进入弃牌堆；单位实例死亡不回手牌。
...
| # | 兵 ID | 战术角色 | 攻击模式 | 主动技能 | 稀有度 | 人口 |
|---|-------|---------|---------|---------|-------|------|
| 1 | `shield_guard` | 肉盾 | `single_target` | **嘲讽**（强制周围敌人攻击自己，3s） | Common | 见 NUMBERS |
| 2 | `swordsman` | 前排输出 | `single_target` | **旋风斩**（80px AOE 30 伤害） | Common | 见 NUMBERS |
| 3 | `archer` | 远程 DPS | `single_target` | **狙击**（400px 单体 ATK×3） | Common | 见 NUMBERS |
| 4 | `priest` | 治疗支援 | `heal` | **治疗链**（150px 范围持续回血） | Rare | 见 NUMBERS |
| 5 | `engineer` | 修理建造 | `single_target` | **紧急修复**（修复周围建筑 50% HP） | Rare | 见 NUMBERS |
| 6 | `assassin` | 近战爆发 | `single_target` | **暗杀**（瞬移到最弱敌人旁高伤） | Epic | 见 NUMBERS |

所有士兵：`category: Soldier`，可移动，持续占用人口（`energyCost` 复用为持续人口占用）。死亡后人口释放。
...
| `energy_crystal` | 下波开始 +3E / +1 能量上限 | L3 | Rare |
...
| `ancient_altar` | 300 | 每 20s 随机给场上 1 个单位 +1 instanceLevel（本波） |
...
| 单位 ID | 主题限定 | 触发 | 效果 |
|---------|---------|------|------|
| `explosive_barrel` | 火山/王城 | 点击/敌人触碰/链锁 | r100 AOE 200 物理；可连锁 |
| `boulder_perch` | 雪山/王城 | 点击或塔射击 | 沿路径滚落 3 格，单体 150 |
...
获卡/购卡后进入卡池（关外每种 1 张上限），获卡=自动解锁对应科技树节点。
