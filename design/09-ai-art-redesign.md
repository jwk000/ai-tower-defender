...
| 场景装饰 | `public/art/decor/` | `decor_<theme>_<decor_id>_idle_<variant>.png` |
...
| 场景装饰 | 128×128 | 是 | 障碍/装饰物静态变体贴图，锚点居中落在占用地格中心；动态表现由运行时粒子实现 |
| 战斗背景 | 1920×1080 | 否 | 分层时用 far/mid/overlay |

敌方单位和 Boss 的透明背景必须同时清理外边缘背景与小面积内部残留，包含腋下、手心、腿间等被身体轮廓围住的位置；允许保留角色自身高光和半透明特效，但不能残留白底、棋盘底或地格底色块。

敌方单位战斗帧源图统一以面朝左为默认朝向，运行时 `Visual.facing=-1` 时保持原图，`Visual.facing=1` 时水平翻转；巨型机器人、超级机器人、弗兰肯斯坦、草原巨人、巨型史莱姆、虫族女王、路西法、深渊领主同样遵循该规则。
...
| `move` | 2 | 敌人、士兵、飞行召唤物 | 左右脚/翼位/身体前倾切换；塔、生产建筑、机关不生成移动动作 |
| `attack` | 2 | 有攻击能力单位、可触发机关 | 蓄力帧 + 释放帧；塔/建筑主体不位移，仅变化武器、能量、炮口/核心火光等自身发射细节，不把子弹、箭矢、导弹、光束等投射物画进塔帧；地刺、捕兽夹、焦油坑的 `attack_0` 必须是收起/待触发状态，`attack_1` 必须是发出/触发峰值状态；巨石只能被攻击，不生成攻击动作 |
...
卡牌资产必须同时生成 **卡牌插画** 与 **整卡 UI 皮肤** 。插画负责识别单位/技能，整卡 UI 皮肤用于抽卡等大卡展示；手牌直接使用卡牌图鉴式运行时绘制，不叠加整卡图像底框。

### 4.1 卡牌 UI 结构

| 区域 | 美术要求 |
|------|----------|
| 外框 | 手牌不使用外框贴图，抽卡等大卡展示统一使用普通黑铁材质外框，不按稀有度切换资源；圆角不超过 8px |
...
调试面板必须提供“单位动作预览”入口，用于检查所有单位动作帧是否齐全且播放稳定。预览窗口按“塔”“士兵”“第 N 关敌人”分页展示，关卡敌人页只列出该关波次中出现过的敌人并去重。窗口顶部提供 `idle`、`move`、`attack`、`death` 动作切换；点击攻击动作时，当前页所有单位同步循环播放 `attack_0/1`，点击死亡动作时同步播放 `death_0/1`。缺少当前动作帧时，卡片回退显示 `idle` 帧并标注缺帧状态；塔的 `move` 缺帧属于预期，不作为错误。该预览只读取 `public/art/units/unit_<unit_id>_<state>_<frame>.png` 资源，不生成 ECS 实体，不改变战斗状态、数值、寻路或渲染管线。

机关资产按被动状态机交付，不按普通单位补齐无意义动作：
...
- `attack`: `Visual.attackAnimTimer > 0` 时优先播放，`attack_0` 为蓄力/后撤帧，`attack_1` 为释放/前冲帧。
- `death`: `LifecycleSystem` 创建死亡特效实体时保留原单位图片资源 id，在死亡特效持续时间内播放，`death_0` 为濒死完整帧，`death_1` 为破碎/消散帧。
- `move`: 可移动单位实际移动且未攻击、死亡、眩晕、冻结时播放，`move_0` / `move_1` 为两步行进或飞行拍翼帧；机关和建筑静止单位不请求 `move`。
- `idle`: 其他时间播放，继续通过呼吸相位在 `idle_0` / `idle_1` 间切帧。

超级机器人的 `move_0` / `move_1` 必须表现为朝左的重型机械平稳行进：沿用基准图的黑钢装甲、肩部导弹舱、尖刺肩甲、红色核心和侧刃轮廓，不允许重绘成其他机器人；两帧必须有明显腿部迈步和手臂摆动，但头部、肩线、胸腔红色核心和整体躯干保持稳定，不允许整机倾斜、旋转或斜向压缩。

`hit`、`skill`、`disabled` 仍属于后续扩展状态；在对应图片未接入前继续复用已有受击、眩晕、冰冻、中毒、精英描边、Boss 阶段等程序化状态特效。
...
| 动态需求 | 适用装饰 | 静态变体与实现方式 |
|----------|----------|----------|
| 无动态 | 岩石、枯骨、残柱、碎石、箱子、墙体、雕像、煤堆 | 1 张静态图直接绘制 |
| 轻微生命感 | 树、灌木、花丛、仙人掌、藤蔓、蘑菇、雪堆 | 1-2 张静态变体 + 运行时 `leaf_motes` / 花粉微粒 |
| 火光/能量 | 火盆、紫火、熔岩口、虫洞、虚空裂隙、扭曲现实 | 1-4 张静态变体 + 运行时 `ember_motes` / `rift_sparks` |
| 机械/蒸汽 | 齿轮、蒸汽管、传送带、废土机械装饰 | 1 张静态图 + 运行时 `steam_puffs` / 火花短线 |
...
背景保持暗黑氛围，但棋盘区域不得有高频细节。背景交付格式固定为 `1920x1080` WebP；如果图片生成模型不支持 `1920x1080` 精确尺寸，必须优先生成同模型高分辨率近似 16:9 原图，再做最小裁切到 `1920x1080`，禁止用旧低分辨率素材放大替代重新生成，禁止拉伸变形。
...
| 手牌区 | `ui_hand_panel`、`ui_card_frame_*`、空槽、拖拽高亮、选中框 | 底座轻量，不复用厚重大面板；手牌卡面使用卡牌框图片且不半透明；卡牌插画位于卡框下层，卡框内容区由运行时遮罩透出插画，不允许插画盖住边框；卡框内容区保持深色，不出现白色或浅灰留白；卡槽清楚 |
...
| 塔投射物 | 已接入，箭塔普通箭矢、炮塔炮弹、冰塔冰晶弹、弩塔弩矢、导弹塔导弹、火塔火球、毒塔毒液弹优先使用 FX 透明贴图；贴图未加载时必须回退程序化几何弹体，不能空渲染 | `RenderSystem` + `imageCache` |
| 士兵投射物 | 已接入，弓手长箭、法师奥术弹、牧师神圣弹优先使用 FX 透明贴图；贴图未加载时必须回退程序化几何弹体，不能空渲染 | `RenderSystem` + `imageCache` |
