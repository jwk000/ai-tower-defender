...
| `attack` | 2 | 有攻击能力单位、可触发机关 | 蓄力帧 + 释放帧；塔/建筑主体不位移，仅变化武器、能量、炮口/核心火光等自身发射细节，不把子弹、箭矢、导弹、光束等投射物画进塔帧；地刺、捕兽夹、焦油坑的 `attack` 表示触发动作；巨石只能被攻击，不生成攻击动作 |
...
当前单位图片接入基线为普通可移动/可攻击场景单位至少交付 `idle_0` / `idle_1`、`move_0` / `move_1`、`attack_0` / `attack_1`、`death_0` / `death_1` 四个状态共 8 帧。`idle` 可通过轻量缩放/呼吸辅助表现；`move`、`attack`、`death` 必须使用重新绘制的动作帧，不允许只用缩放、拉伸、旋转基准图替代。塔和生产建筑属于建筑类单位，`attack` 帧不得改变底座、主体锚点或整体重心，只允许追加/强化塔自身的炮口火光、弓弦蓄光、晶体亮度、线圈电光、毒囊气泡、火焰亮度、发射管热光等细节；子弹、箭矢、导弹、飞出毒液、激光束等投射物由运行时投射物系统绘制，不允许烘焙进塔攻击帧。

机关资产按被动状态机交付，不按普通单位补齐无意义动作：

| 机关 | 必交状态 | 不生成状态 | 运行时触发规则 |
|------|----------|------------|----------------|
| 地刺 | `idle_0/1`、`attack_0/1`、`death_0/1` | `move` | `Trap.animTimer > 0` 时播放 `attack`，表现尖刺弹出/回落 |
| 捕兽夹 | `idle_0/1`、`attack_0/1`、`death_0/1` | `move` | `Trap.animTimer > 0` 时播放 `attack`，表现夹齿合拢 |
| 焦油坑 | `idle_0/1`、`attack_0/1`、`death_0/1` | `move` | `Trap.animTimer > 0` 时播放 `attack`，表现焦油鼓泡/黏液牵引 |
