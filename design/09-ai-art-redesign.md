...
当前单位图片接入基线为所有场景单位至少交付 `idle_0` / `idle_1`、`move_0` / `move_1`、`attack_0` / `attack_1`、`death_0` / `death_1` 四个状态共 8 帧。`idle` 可通过轻量缩放/呼吸辅助表现；`move`、`attack`、`death` 必须使用重新绘制的动作帧，不允许只用缩放、拉伸、旋转基准图替代。运行时状态选择规则如下：
