...
| `attack` | 2 | 有攻击能力单位 | 蓄力帧 + 释放帧；塔/建筑主体不位移，仅变化武器、能量、火光、弹体等发射细节 |
...
当前单位图片接入基线为所有场景单位至少交付 `idle_0` / `idle_1`、`move_0` / `move_1`、`attack_0` / `attack_1`、`death_0` / `death_1` 四个状态共 8 帧。`idle` 可通过轻量缩放/呼吸辅助表现；`move`、`attack`、`death` 必须使用重新绘制的动作帧，不允许只用缩放、拉伸、旋转基准图替代。塔和生产建筑属于建筑类单位，`attack` 帧不得改变底座、主体锚点或整体重心，只允许追加/强化炮口火光、箭矢射出、能量束、电弧、毒雾、火焰、导弹尾焰等细节。运行时状态选择规则如下：
...
- `move`: 可移动单位实际移动且未攻击、死亡、眩晕、冻结时播放，`move_0` / `move_1` 为两步行进或飞行拍翼帧。
