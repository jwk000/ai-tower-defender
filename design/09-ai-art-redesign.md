...
当前单位图片接入基线为所有场景单位至少交付 `idle_0` / `idle_1`、`attack_0` / `attack_1`、`death_0` / `death_1` 三个状态共 6 帧。运行时状态选择规则如下：
...
`move`、`hit`、`skill`、`disabled` 仍属于后续扩展状态；在对应图片未接入前继续复用已有移动、攻击、受击、眩晕、冰冻、中毒、精英描边、Boss 阶段等程序化状态特效。
