...
| 来源 | 说明 | 覆盖 |
|------|------|------|
| **音频文件** | `public/sfx/` 中的外部音频文件 | 97 个 SFX 键 |
| **SoundSynth 合成** | `src/utils/SoundSynth.ts` 程序化生成 WAV | 备用波形生成器 |
...
| `enemy_attack` | 敌人近战攻击 | 敌人普攻，系统级触发 | OGG + Synth |
...
| `soldier_attack` | 士兵攻击 | 剑刃/武器挥击，近战与物理远程士兵实际出手时播放 | OpenGameArt CC0 OGG |
| `soldier_death` | 士兵阵亡 | 单位死亡哀嚎，系统级触发 | OGG + Synth |
...
| 士兵 (4 种) | 4/4 | ✅ 全部有 OGG 文件 |
| **合计** | **97** | ✅ 全部覆盖 |

### 4.1.1 2026-06-19 战斗单位音效缺口与处理

| 缺口 | 原状态 | 处理 |
|------|--------|------|
| 士兵攻击 | 无 `soldier_attack` SFX key，也无实体文件；只有法师 YAML 通过 `SFX_MAGIC_SHOOT` 触发魔法攻击音 | 新增 `soldier_attack.ogg`，近战/物理士兵实际出手时系统级播放；法师/牧师复用 `mage_attack` |
| 士兵死亡 | 有 `soldier_death.ogg`，但普通士兵 YAML `onDeath` 只配置死亡特效，缺音效触发 | `LifecycleSystem` 在士兵死亡时系统级播放 `soldier_death` |
| 敌方单位攻击 | 有 `enemy_attack.ogg`，移动/技能攻击路径已有触发，但普通攻击依赖具体系统路径 | 保留系统级攻击路径触发，并在文档标注为系统级触发 |
| 敌方单位死亡 | 有 `enemy_death*.ogg`，多数 YAML 有 `SFX_ENEMY_DIE`，但仍可能因漏配无声 | `LifecycleSystem` 在敌方死亡时系统级播放类型化死亡音效 |
...
| **合计运行时音频** | 115 个声明键 | ~24 MB |

---

> **版本**: v1.4 — 补齐战斗单位攻击/死亡音效触发
> **最后更新**: 2026-06-19
