...
将大量小尺寸 PNG 资源从“按文件逐张加载”改为“图集 PNG + JSON 帧索引”加载，减少关卡进入时的 HTTP 请求数和图片解码次数，并让缺失图片资源直接暴露为空主体，避免程序化占位外观被误认为正式美术。

运行时必须满足：

1. 业务层继续使用原始资源路径，如 `/art/units/unit_enemy_goblin_idle_0.png`。
2. 加载层根据图集 manifest 自动把原路径解析为图集帧。
3. manifest 缺失或图集图片未加载时，可回退尝试原单图；原单图也不可用时不绘制对应图片主体。
...
`player_units` 仅包含士兵、机关、生产建筑等非塔玩家单位。它们跨关卡复用，不能按关卡重复打包；若单张超过尺寸上限，可拆为 `player_units_01`、`player_units_02` 等连续分片，但仍不得与敌人或塔混包。

`fx_objectives` 包含技能 FX、传送门、水晶、状态小图标等战斗通用资源。正式子弹贴图必须由 AI 生成源图后去底裁切，最终 PNG 最大边不超过 `60px`；FX 图集打包时必须保持每张资源的实际宽高，不能强制拉伸为正方形。例如导弹塔投射物 `fx_missile_projectile.png` 当前为 `60×20`，图集帧也必须保持 `60×20`，否则运行时会把弹头裁掉或压成细线。
...
- 抠背景基于图片边缘采样，先移除与边缘连通的白色、棋盘、地格或其他生成背景，再清理小面积内部残留背景；内部残留清理只作用于与背景色接近的封闭小区域，避免误删物件内部高光。
...
| 技能 FX | WebP | 最大 256×256 等比缩放；大范围特效最多 512×512 | 当前多数 1024×1024 PNG 过大，需降尺寸；非方形投射物必须保留宽高比 |
...
1. 单张图集最大边长 2048；移动端或低端设备优先控制在 1024。敌方单位必须按关卡生成 `level_xx_enemies` 图集，单关过大时允许拆为 `level_xx_enemies_01` 等连续分片；禁止再使用 `units_01`、`units_02` 这类跨场景混合图集。塔图集必须每塔一张，打包宽度上限为 1024，6 个 256×256 帧通常排为 2 行，单张尺寸约 776×518（含 padding），不得再与其他单位混包。主题图集必须合并同主题地格和场景装饰，禁止把装饰物另拆为全局图集。
...
2. 玩家在关卡选择界面点击开始关卡后，先进入加载态，不立即创建战斗世界。
...
GitHub Pages 首次访问往往受静态资源冷缓存、跨区域网络和图片解码影响。关卡入口必须阻塞等待本关关键图集，避免进关后第一波才触发大量图片请求造成卡顿；但不得在普通关卡入口预加载所有关卡敌人图集，避免第五关以外的资源拖慢低关卡启动。
...
4. 图集和单图均缺失时，对应图片主体不绘制，战斗逻辑不受影响。
