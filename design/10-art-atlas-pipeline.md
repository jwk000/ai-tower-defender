...
每个塔图集只包含对应 `unit_tower_<type>_*` 的 `idle`、`move`、`attack`、`death` 等帧，目标尺寸优先控制在 1024 以下，避免全部塔混入一个大图集。
...
1. 单张图集最大边长 2048；移动端或低端设备优先控制在 1024。塔图集必须每塔一张，打包宽度上限为 1024，8 个 256×256 帧通常排为 3 行，单张尺寸约 776×776（含 padding），不得再与其他单位混包。
